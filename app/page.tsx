...
export default function HomePage() {
  ...
  const handleSearch = async (customQuery?: string) => {
    ...
    try {
      ...
      // SAVE TO HISTORY: Runs only if user is logged in
      if (user) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative mb-8">
            <input 
              ...
              placeholder="Search e.g. 'MX Master 3S' or 'Boox Air 3'..."
            />
            <button 
          ...
          {isAnalyzing && (
            <div className="text-center py-24">
              ...
              <p className="text-indigo-400 font-bold tracking-[0.2em] uppercase text-sm">Auditing Market Data...</p>
            </div>
          )}
          ...
          {analysis && (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
              ...
              <div className="bg-white/5 border border-white/10 rounded-[40px] p-8 md:p-12 shadow-2xl">
                <div className="grid md:grid-cols-2 gap-12 items-start">
                  ...
                  <div>
                    ...
                    {/* PUBLIC SUMMARY (AI Consensus) */}
                    <div className="mb-8 p-6 bg-white/[0.03] rounded-3xl border border-white/5">
                      <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-3">AI Market Consensus</h4>
                      <p className="text-gray-400 text-sm leading-relaxed italic">
                        "{analysis.main_product?.public_summary || "Gathering technical specifications and user reviews..."}"
                      </p>
                    </div>

                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Live Market Deals</h4>
                    ...
                    <div className="space-y-4 mt-8">
                      ...
                      {analysis.main_product?.used_deals?.map((d: any, i: number) => (
                        <a key={i} href={d.url} target="_blank" rel="noopener noreferrer" className="flex justify-between items-center p-5 bg-emerald-500/5 rounded-2xl border border-emerald-500/10 hover:border-emerald-500 transition group">
                          <span className="font-bold text-sm">{d.vendor} (Used)</span>
              ...
              {/* PREMIUM REASONING (The "Suits Me" Section) */}
              <section className="bg-indigo-600/10 border border-indigo-500/20 rounded-[40px] p-10 relative overflow-hidden shadow-2xl">
                 <h3 className="text-xl font-bold mb-6 text-indigo-300 italic tracking-tight uppercase">Why this suits you:</h3>
                 <div className={!user ? "filter blur-2xl opacity-10 pointer-events-none select-none" : "opacity-100"}>
                   <div className="text-lg leading-relaxed text-indigo-50 font-medium whitespace-pre-line space-y-4">
                     {analysis?.suits_me_reason || "Log in to see your personalized compatibility report."}
                   </div>
                 </div>
                 {!user && (
                   <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md transition-all">
                     <div className="text-center p-8">
                       <p className="font-bold mb-6 text-sm text-gray-300 uppercase tracking-[0.3em]">Personalized Audit Locked</p>
